...
if (!isset($_GET['listing_edit']) && $paid_submission_status == 'membership' && $remaining_listings != -1 && $remaining_listings < 1) {
    ...
    <form id="new_post" name="new_post" method="post" action="" enctype="multipart/form-data" class="row add-estate">
        ...
        <!-- Ofiliaria inicio cambios -->
        <div id="ofiliaria_gif_espere" style="text-align: center;"></div>
        <div class="col-md-12 row_dasboard-prop-listing">
            <div id="ofiliaria_mensaje_validacion" class="alert alert-danger"></div>
        </div>
        <!-- Ofiliaria inicio cambios -->
        ...
        <?php     $get_listing_edit = isset($_GET['listing_edit']) ? intval($_GET['listing_edit']) : ''; ?>
            ...
            <div class="col-12 col-md-12 col-lg-7 wpestate_dash_coluns">
                <div class="wpestate_dashboard_content_wrapper">
                    ...
                    // Ofiliaria inicio cambios
                    include(locate_template('templates/submit_templates/ofiliaria_publicar_meli.php') );
                    include(locate_template('templates/submit_templates/ofiliaria_publicar_infocasas.php') );
                    // Ofiliaria fin cambios
                    ?>
                    <?php
                    if ( !wp_is_mobile() ) {  ?>
                        <div class="profile-onprofile row submitrow">
                            ...
                            <input type="hidden" name="is_save_draft" id="is_save_draft" value="0">
                            <input type="hidden" name="is_submit_prop" id="is_submit_prop" value="0">

                            <?php if ($action == 'edit') : ?>
                                <input type="submit" class="wpresidence_button" name="submit_prop" id="form_submit_1" 
                                    onclick="document.getElementById('is_submit_prop').value='1';" 
                                    value="<?php esc_attr_e('Save Changes', 'wpresidence') ?>" />
                                    
                                <input type="submit" class="wpresidence_button" name="save_draft" id="form_submit_2" 
                                    onclick="document.getElementById('is_save_draft').value='1';" 
                                    value="<?php esc_attr_e('Save as Draft', 'wpresidence') ?>" />
                            <?php else : ?>
                                <input type="submit" class="wpresidence_button" name="submit_prop" id="form_submit_1" 
                                    onclick="document.getElementById('is_submit_prop').value='1';" 
                                    value="<?php esc_attr_e('Add Property', 'wpresidence') ?>" />
        ...
        if ( wp_is_mobile() ) { ?>
            ...
            <div class="profile-onprofile row m-0 p-3  submitrow">
                ...
                <input type="hidden" name="is_save_draft" id="is_save_draft_mobile" value="0">
                <input type="hidden" name="is_submit_prop" id="is_submit_prop_mobile" value="0">

                <?php if ($action == 'edit') : ?>
                    <input type="submit" class="wpresidence_button mx-0" style="width:100%;" name="submit_prop" id="form_submit_1_mobile" 
                        onclick="document.getElementById('is_submit_prop_mobile').value='1'; document.getElementById('is_save_draft_mobile').value='0';" 
                        value="<?php esc_attr_e('Save Changes', 'wpresidence') ?>" />
                        
                    <input type="submit" class="wpresidence_button mx-0" style="width:100%;" name="save_draft" id="form_submit_2_mobile" 
                        onclick="document.getElementById('is_save_draft_mobile').value='1'; document.getElementById('is_submit_prop_mobile').value='0';" 
                        value="<?php esc_attr_e('Save as Draft', 'wpresidence') ?>" />
                <?php else : ?>
                    <input type="submit" class="wpresidence_button mx-0" style="width:100%;" name="submit_prop" id="form_submit_1_mobile" 
                        onclick="document.getElementById('is_submit_prop_mobile').value='1'; document.getElementById('is_save_draft_mobile').value='0';" 
