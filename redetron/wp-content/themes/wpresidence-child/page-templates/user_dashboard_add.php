    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($parent_userID) > 0)  ){ // if user can submit
        ...
        */
        ...
        if($has_errors){
            ...
            // Inicio cambios Ofiliaria
            error_log( 'Valor de is_submit_prop: ' . (isset($_POST['is_submit_prop']) ? $_POST['is_submit_prop'] : 'no llega') );
            error_log( 'Valor de is_save_draft: ' . (isset($_POST['is_save_draft']) ? $_POST['is_save_draft'] : 'no llega') );
            // Fin cambios Ofiliaria

            if( isset($_POST['is_save_draft']) && $_POST['is_save_draft'] == '1' ){
                $new_status = 'draft';
            }
        ...
        if($post_id) {
            ...
            $estatus_publicacion_meli = 'No publicada';
            if (!empty($_POST['estatus_publicacion_meli']))
            {
                $estatus_publicacion_meli = $_POST['estatus_publicacion_meli'];
            }

            do_action('agregar_publicacion_meli', $current_user, $identificador, $post_id, $_POST['id_publicacion_meli'], $estatus_publicacion_meli, $new_status );
            do_action('agregar_publicacion_infocasas', $current_user, $identificador, $post_id);
            // Fin cambios Ofiliaria
...
if( isset($_POST) && isset($_POST['action'])  &&  $_POST['action']=='edit' ) {
        ...
        update_post_meta(1, 'user_dashboard_add action edit', 'Sí');
        ...
        */
        ...
        if( $edited==1) {
            ...
            do_action('editar_publicacion_meli', $current_user, $identificador, $post_id, $_POST['id_publicacion_meli'], $estatus_publicacion_meli, $new_status );
            do_action('editar_publicacion_infocasas', $current_user, $identificador, $post_id );
            // Fin cambios Ofiliaria
