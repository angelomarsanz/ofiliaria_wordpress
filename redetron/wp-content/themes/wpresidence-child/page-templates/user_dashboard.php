...
if (isset($_GET['delete_id'])) {
    ...
    do_action('eliminar_publicacion_meli', $_GET['delete_id'], $current_user);
...
<div class="row row_user_dashboard">
    ...
    <div class="col-md-9 dashboard-margin row">
        ...
        // Comprobar si existe un mensaje flash
        $flash_message_data = get_transient( 'wpestate_flash_message_' . $userID );
        if ( ! empty( $flash_message_data ) && is_array( $flash_message_data ) && isset( $flash_message_data['message'], $flash_message_data['type'] ) ) {
            $message_type    = $flash_message_data['type'] === 'success' ? 'success' : 'danger'; // Sanitize to success or danger
            $message_text    = $flash_message_data['message'];
            $bootstrap_class = 'alert alert-' . esc_attr( $message_type );

            echo '<div class="' . $bootstrap_class . '">' . $message_text . '</div>';
            // Eliminar el transient para que no se muestre de nuevo
            delete_transient( 'wpestate_flash_message_' . $userID );
        }
