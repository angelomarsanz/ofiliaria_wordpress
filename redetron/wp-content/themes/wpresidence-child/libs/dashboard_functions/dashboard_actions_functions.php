...
if( !function_exists('wpestate_disable_listing') ):
    function wpestate_disable_listing(){
        ...
        do_action('pausar_activar_publicacion_meli', $prop_id, $new_status, $current_user);
...
if( !function_exists('wpestate_dashboard_property_list') ):
  function wpestate_dashboard_property_list($agent_list,$order_by='0',$status_value='0'){
      ...
      print '</div>';
      // Inicio cambios Ofiliaria
      print '<div id="indexListaPropiedades"></div>';
      $agentes_agencia = [];
      $usuario_conectado = wp_get_current_user();
      $id_post_agencia_agente_usuario_conectado = get_user_meta($usuario_conectado->ID, 'user_agent_id', true);
      $post_agencia_agente_usuario_conectado = get_post($id_post_agencia_agente_usuario_conectado);
      $id_autor_usuario_conectado = $post_agencia_agente_usuario_conectado->post_author;
      if ($id_autor_usuario_conectado == 0)
      {
        $agentes_agencia = apply_filters('ofiliaria_filtro_agentes_agencia', $agentes_agencia, $usuario_conectado->ID);  
      }
