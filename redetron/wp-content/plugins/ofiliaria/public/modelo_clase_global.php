...
class Redet_As_0001_Public {
	...
	public function __construct( $redet_as_0001, $version ) 
	{
		...
		add_action( 'wp_ajax_verificar_token_sbi', array( $this, 'verificar_token_sbi') );
		add_action( 'wp_ajax_nopriv_verificar_token_sbi', array( $this, 'verificar_token_sbi') );
		add_action( 'wp_ajax_actualizar_token_sbi_bd', array( $this, 'actualizar_token_sbi_bd') );
		add_action( 'wp_ajax_nopriv_actualizar_token_sbi_bd', array( $this, 'actualizar_token_sbi_bd') );

		/**
		 * [id_producto_sbi] Retorna el id del producto
		 * @return string id del producto
		*/

		add_shortcode( 'id_producto_sbi', array( $this, 'controlador_id_producto_sbi' ));

		/**
		 * [id_paquete_sbi] Retorno el id del paquete
		 * @return string id del paquete
		*/

		add_shortcode( 'id_paquete_sbi', array( $this, 'controlador_id_paquete_sbi' ));

		/**
		 * [nombre_producto_sbi] Retorna el nombre del producto
		 * @return string nombre del producto
		*/

		add_shortcode( 'nombre_producto_sbi', array( $this, 'controlador_nombre_producto_sbi' ));

		/**
		 * [nombre_paquete_sbi] Retorna el nombre del paquete
		 * @return string nombre del paquete
		*/

		add_shortcode( 'nombre_paquete_sbi', array( $this, 'controlador_nombre_paquete_sbi' ));
	...
	public function enqueue_scripts() 
	{
		$paginas_crm = 
			[
				...
				"hpage=deals",
				"hpage=enquiries",
				"hpage=detalle_solicitud_otro_agente",
				"hpage=garantias",
				"hpage=informes",
				"hpage=solicitudes_otros_agentes",
				"hpage=activities"
			];
		...
		wp_enqueue_script( 'redet-as-0001-frontend', plugin_dir_url( __FILE__ ) . 'js/redet-as-0001-public-frontend.js', array( 'jquery'), time(), false );
		wp_localize_script('redet-as-0001-frontend', 'Houzez_crm_vars_redet_as', $locals_redet_as ); 

		$url_actual = $_SERVER['REQUEST_URI'];

		$indicador_encontrado = 0;

		foreach ($paginas_crm as $pagina)
		{
			$posicion_cadena = strpos($url_actual, $pagina);
			if ($posicion_cadena !== false) 
			{
				$indicador_encontrado = 1;
				break;
			}
		}

		if ($indicador_encontrado == 1)
		{
			update_post_meta(1, 'url_actual', 'La URL '.$url_actual.' está en la lista');
		...
		{
			update_post_meta(1, 'url_actual', 'La URL '.$url_actual.' no está en la lista');
		}
	}
	...
	public function buscar_adjuntos_informe($id_informe = null)
	{
		...
		if ($total_adjuntos > 0):
			...
			foreach ($adjuntosInformes as $adjunto):
				$post_adjunto = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'posts WHERE ID = '. $adjunto['valor'], OBJECT );
				if( is_object( $post_adjunto ) && ! empty( $post_adjunto ) ) 
				{
					$extension = substr($post_adjunto->post_mime_type, strpos($post_adjunto->post_mime_type, '/') + 1);
					$adjuntos_informe[] = 
						[
	...
	public function verificar_token_sbi() 
	{
		$indicador_token_vencido = 0;
		$token_sbi_bd = "";
		setlocale(LC_TIME, 'es_UY', 'es_UY.UTF-8', 'es_UY.UTF-8'); 
		date_default_timezone_set('America/Montevideo');

		$fecha_hora_token_sbi_bd = get_option('_fecha_hora_token_sbi');
		$fecha_hora_token_sbi_objeto = new DateTime($fecha_hora_token_sbi_bd );
		$fecha_hora_actual_objeto = new DateTime('now');
		$intervalo_diferencia = $fecha_hora_token_sbi_objeto->diff($fecha_hora_actual_objeto);

		if ($intervalo_diferencia->y > 0)
		{
			$indicador_token_vencido = 1;
		}
		...
		elseif ($intervalo_diferencia->h > 7)
		{
			$indicador_token_vencido = 1;
		}

		if ($indicador_token_vencido == 0)
		{
			$token_sbi_bd = get_option('_token_sbi');
		}
	
		echo json_encode( array(
			'indicador_token_vencido' => $indicador_token_vencido,
			'fecha_hora_token_sbi_bd' => 'fecha_hora_token_sbi_bd '.$fecha_hora_token_sbi_bd,
			'token_sbi_bd' => $token_sbi_bd 
		));
		die;
	}
	public function actualizar_token_sbi_bd()
	{
		setlocale(LC_TIME, 'es_UY', 'es_UY.UTF-8', 'es_UY.UTF-8'); 
		date_default_timezone_set('America/Montevideo');

		$nuevo_token = $_POST['nuevo_token'];
		$fecha_hora_token_sbi_actualizada = date("Y-m-d H:i:s");

		update_option('_token_sbi', $nuevo_token);
		update_option('_fecha_hora_token_sbi', $fecha_hora_token_sbi_actualizada);
	
		echo json_encode( array(
			'resultado' => true,
			'mensaje' => ' Fecha hora actualizada token SBI: '.$fecha_hora_token_sbi_actualizada
		));
		die;
	}
	
	public function controlador_id_producto_sbi() 
	{
		global $wpdb;
		$tabla_productos = $wpdb->prefix.'productos_sbi';
		$mensaje_retorno = "El id del producto no existe";
		
		if (isset($_GET['producto']))
		{
			$nombre_producto = $_GET['producto'];
			$query_producto = "SELECT * FROM {$tabla_productos} WHERE nombre_producto = '{$nombre_producto}'";

			$producto_sbi = $wpdb->get_row($query_producto, OBJECT );
			if (is_object( $producto_sbi ) && ! empty( $producto_sbi )) 
			{
				$mensaje_retorno = $producto_sbi->id;
			}
		}
		return $mensaje_retorno;
	}

	public function controlador_id_paquete_sbi() 
	{
		global $wpdb;
		$tabla_paquetes = $wpdb->prefix.'paquetes_sbi';
		$mensaje_retorno = "El id del paquete no existe";

		$id_producto = $this->controlador_id_producto_sbi();
		
		if ($id_producto != "El id del producto no existe" && isset($_GET['paquete']))
		{
			$nombre_paquete = $_GET['paquete'];
			$query_paquete = "SELECT * FROM {$tabla_paquetes} WHERE nombre_paquete = '{$nombre_paquete}' AND id_producto = '{$id_producto}'";

			$paquete_sbi = $wpdb->get_row($query_paquete, OBJECT );
			if (is_object( $paquete_sbi ) && ! empty( $paquete_sbi )) 
			{
				$mensaje_retorno = $paquete_sbi->id;
			}
		}
		return $mensaje_retorno;
	}

	public function controlador_nombre_producto_sbi() 
	{
		$mensaje_retorno = "El nombre del producto no existe";
		
		if (isset($_GET['producto']))
		{
			$mensaje_retorno = $_GET['producto'];
		}
		return $mensaje_retorno;
	}

	public function controlador_nombre_paquete_sbi() 
	{
		$mensaje_retorno = "El nombre del paquete no existe";
		
		if (isset($_GET['paquete']))
		{
			$mensaje_retorno = $_GET['paquete'];
		}
		return $mensaje_retorno;
	}
