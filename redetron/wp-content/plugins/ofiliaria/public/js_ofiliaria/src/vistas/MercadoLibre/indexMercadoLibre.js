...
export const indexMercadoLibre = () => 
{
    (function( $ ) {
        if ($('#indexMercadoLibre').length > 0) 
        {
            ...
            var errorTokenMeliConfiguracionConexion = "";
            var mensajeErrorTokenMeliConfiguracionConexion = "";
            ...
            function ejecutarErrorTokenMeli(error_token_meli, mensaje_error_token_meli)
            {
                    errorTokenMeliConfiguracionConexion = error_token_meli;
                    mensajeErrorTokenMeliConfiguracionConexion = mensaje_error_token_meli;
                    console.log('errorTokenMeliConfiguracionConexion', errorTokenMeliConfiguracionConexion);
                    console.log('mensajeErrorTokenMeliConfiguracionConexion', mensajeErrorTokenMeliConfiguracionConexion);
                    errorTokenMeli(errorTokenMeliConfiguracionConexion, mensajeErrorTokenMeliConfiguracionConexion, ajaxurl);
            }
            ...
            async function buscarTokenBaseDatos() {
                ...
                $("#configuracion_conexion_meli").text(`¡ Estimado usuario por favor espere un momento, estamos verificando si su conexión con Mercado Libre está activa... !`);
                ...
                if (codigo_respuesta == 4)
                {
                    ...
                    else
                    {
                        ...
                        ejecutarErrorTokenMeli(respuestaRefrescarToken.error, respuestaRefrescarToken.mensaje_error);
                    }
                }
                else
                {
                    otrosResultados(codigo_respuesta, mensaje_respuesta);
                }
            ...
            function otrosResultados(codigo_respuesta, mensaje_respuesta) 
            {
                ...
                {
                    $("#configuracion_conexion_meli").text(`¡ Estimado usuario usted no tiene rol de agencia o agente !`);
                }
                else if (codigo_respuesta == 2)
                {
                    $("#configuracion_conexion_meli").text(`¡ Estimado usuario usted no está asignado a una agencia o agente !`);
                }
                else
                {
                    ...
                    $("#configurar_conexion_meli").show();  
                }
                ejecutarErrorTokenMeli(codigo_respuesta, mensaje_respuesta);
            } 

            function previoImportarPublicacionesMeli() {
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: ajaxurl,
                    data: 
                    {
                        'action' : 'previo_importar_publicaciones_meli'
                    },
                    success: function(data) 
                    {
                        console.log('previoImportarPublicacionesMeli, data', data);
            ...
            /* 
            Se comentó porque ya no se redimensionarán las imágenes porque se pixelan demasiados. Se deja el código como una guía por si más adelante se necesitan redimensionar imágenes hacia abajo, osea achicar la imagen. En ese caso no se redimensiona
            ...
            async function procesarPublicacionesRedimensionImagenesMeli(data) 
            {
                ...
                {
                    //if (data.publicaciones_redimension_imagenes_meli[i].id_post > 13785 && data.publicaciones_redimension_imagenes_meli[i].id_post < 16749 )
                    //{
            ...
            */
            ...
            $(function() {
                ...
                $('#previo_importar_publicaciones_meli').on( 'click', function(event) 
                {
                    event.preventDefault();
                    previoImportarPublicacionesMeli();
                    $('#mensajes_importacion_meli').text('Previo importando publicaciones, por favor espere...');
                });
