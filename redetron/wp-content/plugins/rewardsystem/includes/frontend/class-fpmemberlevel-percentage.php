	...
	class RSMemberFunction {
		...
		public static function earn_points_percentage( $user_id, $points ) {
			...
			if ( ( 'yes' == $EarnLevelBasedonUserRole ) && ( 'yes' != $EarnLevelBasedonPoints ) && ( 'yes' != $EarnLevelBasedonMembership ) ) {
				...
				$percentage      = self::promotional_points( $percentage, $selected_role );
				return $percentage;
			}
		 ...
		 */
		public static function promotional_points( $percentage, $user_role ) {
			...
			foreach ( $rule_ids as $rule_id ) {
				...
				if ( srp_check_is_array( $rule->get_included_user_role() ) ) {
					if (! in_array( $user_role, $rule->get_included_user_role() )) {
						continue;
					}
				}
