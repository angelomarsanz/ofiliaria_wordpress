...
$PointsInOrder    = ( '1' == $emails->rsmailsendingoptions ) ? earned_points_from_order( $order_id ) : redeem_points_from_order( $order_id );
$PointsInTemplate = ( '1' == $emails->rsmailsendingoptions ) ? ( empty( $emails->earningpoints ) ? 0 : $emails->earningpoints ) : ( empty( $emails->redeemingpoints ) ? 0 : $emails->redeemingpoints );
...
$message           = ( '1' == $emails->rsmailsendingoptions ) ? str_replace( array( '{rs_earned_points}', '{order_id}' ), array( $PointsInOrder, $order_id ), $message ) : str_replace( array( '{rs_redeemed_points}', '{order_id}' ), array( $PointsInOrder, $order_id ), $message );
$subject           = ( '1' == $emails->rsmailsendingoptions ) ? str_replace( array( '{rs_earned_points}', '{order_id}' ), array( $PointsInOrder, $order_id ), $subject ) : str_replace( array( '{rs_redeemed_points}', '{order_id}' ), array( $PointsInOrder, $order_id ), $subject );
