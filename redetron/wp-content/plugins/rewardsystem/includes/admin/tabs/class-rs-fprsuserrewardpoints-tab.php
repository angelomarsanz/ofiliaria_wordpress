...
if ( ! class_exists( 'RSUserRewardPoints' ) ) {
	...
	class RSUserRewardPoints {
		...
		public static function reward_system_admin_fields() {
			...
			return apply_filters(
				...
				array(
					...
					array(
						'name' => __( 'Ajustes de Saldo de usuarios', 'rewardsystem' ),
						'type' => 'title',
						'id'   => 'rs_user_reward_points_setting',
					),
					array(
						'name'     => __( 'Filtrar por', 'rewardsystem' ),
						...
						'type'     => 'select',
						'options'  => array(
							'1' => __( 'Usuario(s)', 'rewardsystem' ),
							'2' => __( 'Rol(es)', 'rewardsystem' ),
						),
					),
					array(
						'name'        => __( 'Seleccionar roles de usuario', 'rewardsystem' ),
		...
		public static function user_and_user_role_filter() {
			...
			$field_label = __( 'Seleccionar Usuarios', 'rewardsystem' );
			...
			<tr>
				<td></td>
				<td>
					<input type="submit" value="<?php esc_html_e( 'Buscar', 'rewardsystem' ); ?>" id="rs_submit_for_user_role_log" class="button-primary" name="rs_submit_for_user_role_log">
				</td>
			</tr>
			<?php
		}
		...
		public static function display_log() {
			if ( ( ! isset( $_GET['view'] ) ) && ( ! isset( $_GET['edit'] ) ) ) {
				...
				$user_list_table->search_box( __( 'Buscar Usuarios', 'rewardsystem' ), 'search_id' );
				$user_list_table->display();
			} elseif ( isset( $_GET['view'] ) ) {
				$user_log_list_table = new SRP_View_Log_Table();
				$user_log_list_table->prepare_items();
				$user_log_list_table->search_box( __( 'Buscar', 'rewardsystem' ), 'search_id' );
				$user_log_list_table->display();
				?>
				<a href="<?php echo esc_url( remove_query_arg( array( 'view' ) ) ); ?>"><?php esc_html_e( 'Volver', 'rewardsystem' ); ?></a>
				<?php
			} else {
				...
				?>
				<h3><?php esc_html_e( 'Actualizar saldo de usuarios', 'rewardsystem' ); ?></h3>
				<table class="form-table">
					<tr valign ="top">
						<th class="titledesc" scope="row">
							<label><?php esc_html_e( 'Saldo actual del usuario', 'rewardsystem' ); ?></label>
						</th>
					...
					<tr valign="top">
						<th class="titledesc" scope="row">
							<label><?php esc_html_e( 'Ingresar monto', 'rewardsystem' ); ?></label>
						</th>
						<td class="forminp forminp-text">
					...
					<tr valign="top">
						<th class="titledesc" scope="row">
							<label><?php esc_html_e( 'Referencia/Descripción', 'rewardsystem' ); ?></label>
						</th>
						<td class="forminp forminp-text">
					...
					<tr valign="top">
						<td>
							<input type='submit' name='rs_add_points_for_user' class='button-primary rs_add_point_for_user' value='<?php esc_html_e( 'Acreditar Saldo', 'rewardsystem' ); ?>' />
							<input type='hidden' id ="rs_selected_user_id" value='<?php echo esc_html( $user_id ); ?>'>
						</td>
						<td>
							<input type='submit' name='rs_remove_point_for_user' class='button-primary rs_remove_point_for_user' value='<?php esc_html_e( 'Debitar Saldo', 'rewardsystem' ); ?>' />
						</td>
						<td>
							<a href="<?php echo esc_url( remove_query_arg( array( 'edit', 'saved' ) ) ); ?>">
								<input type='button' class='button-primary' value='<?php esc_html_e( 'Ir Atrás', 'rewardsystem' ); ?>'/>
							</a>
						</td>
					</tr>
				</table>
