...
function init_reward_gateway_class() {
	...
	class WC_Reward_Gateway extends WC_Payment_Gateway {
		...
		public function rs_remove_pending_order( $data, $error ) {
			...
			$Points = !empty( $Points ) ? $Points - $redeeming_coupon : $Points;
		...
		public function validate_fields() {
			...
			$gateway_points = gateway_points( $order_id );
			if ( empty( $gateway_points ) ) {
				return true;
			}

			if ( $Points < $gateway_points ) {
				$error_msg    = $this->get_option( 'error_payment_gateway' );
				$final_message = str_replace( array( '[userpoints]', '[needpoints]' ), array( '<b>' . $Points . '</b>', '<b>' . round_off_type( $gateway_points ) . '</b>' ), $error_msg );
	...
	function gateway_points( $order_id ) {
		...
		$tax_total            = ( 'excl' == get_option( 'woocommerce_tax_display_cart' ) ) ? $order->get_total_tax() : 0 ;
		$shipping_total       = ( 'excl' == get_option( 'woocommerce_tax_display_cart' ) ) ? $order->get_shipping_total() : $order->get_shipping_total() + $order->get_shipping_tax() ;
