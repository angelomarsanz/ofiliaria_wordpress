...
class SRP_Referral_List_Table extends WP_List_Table {
	...
	public function get_sortable_columns() {
		return array(
		'refered_person_count' => array( 'refered_person_count', false ),
			'sno'                  => array( 'sno', false ),
	...
	private function table_data( $startpoint, $perpage ) {
		...
		if ( is_multisite() ) {
			...
			$query_data = array();
			if (srp_check_is_array( $referrer_user_ids )) {
				$query_data = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}users where ID IN('" . implode( "','" , array_filter( $referrer_user_ids ) ) . "')" ) ;
			}
			
			if ( ! srp_check_is_array( $query_data ) ) {
				return array();
			}
