...
if ( ! function_exists( 'get_regular_price' ) ) {
	...
	function get_regular_price( $productid, $variationid, $item, $itemquantity, $payment_price ) {
		...
		if ( '1' === get_option( 'rs_calculate_point_based_on_reg_or_sale' ) ) {
			if ( WC_VERSION >= 3.0 ) {
				$getregularprice = ( (float) $ProductObj->get_price() * $itemquantity ) - srp_get_tax_based_on_item( $item );
			} else {
...
if ( ! function_exists( 'get_post_parent_id' ) ) {
	...
	function get_post_parent_id( $object ) {
		global $woocommerce;
...
if ( ! function_exists( 'srp_product_price' ) ) {
	...
	function srp_product_price( $product ) {
		global $woocommerce;
		if ( (float) $woocommerce->version >= (float) '3.0' ) {
			$price = !empty($product->get_sale_price()) ? $product->get_sale_price() : $product->get_price();
		} else {
