...
if ( ! class_exists( 'RewardPointsOrder' ) ) {
	...
	class RewardPointsOrder {
		...
		public function rs_insert_points_based_on_first_purchase( $enabledisablemaxpoints, $orderuserid ) {
			...
			$order_obj = srp_order_obj( $this->order );
			...
			$order_count = wc_get_orders( $args );
		...
		public function get_reward_points_based_on_product_total_in_referral_system( $args ) {
			...
			if ( 'no' == get_option( 'rs_enable_product_category_level_for_referral_product_purchase' ) && '2' === get_option( 'rs_award_points_for_cart_or_product_total_for_refferal_system', 1 ) ) {
				return 0;
			}
		...
		public function send_sms_and_email_in_product_purchase() {
			...
			if ( 'no' !== $this->apply_previous_order_points || 'yes' == $this->order->get_meta( 'reward_points_awarded' ) ) {
				return;
			}
