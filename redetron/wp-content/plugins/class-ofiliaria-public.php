...
class Ofiliaria_Public {
    ...
    public function __construct( $plugin_name, $version ) 
    {
        ...
        add_action( 'agregar_publicacion_meli', array( $this, 'envio_publicacion_meli'), 10, 5 );
        add_action( 'editar_publicacion_meli', array( $this, 'envio_publicacion_meli'), 10, 5 );
	...
	{
		// Inicio parámetros
		$id_autor = null; // Se colocó null por seguridad por si la acción se ejecuta por accidente
		$id_usuario_meli = '667590561';
		$url = "https://api.mercadolibre.com/users/".$id_usuario_meli."/items/search?status=active&offset=0&limit=2";
		// $url = "https://api.mercadolibre.com/users/".$id_usuario_meli."/items/search?status=active";
		// Fin parámetros
		...
		if ( $id_post_agencia_agente != "")
		{
			...
			if ($indicador_usuario_importacion_meli == '') 
			{
				...
				{
					$respuesta_buscar_publicaciones_meli = $this->buscar_publicaciones_meli($id_autor, $url);	
					if ($respuesta_buscar_publicaciones_meli['codigo_retorno'] == 0)
					{
						...
						foreach ($vector_publicaciones_meli->results as $publicacion)
						{	
							...
							if (empty($contador_publicaciones_sincronizadas))
							{
								...
								if ($respuesta_buscar_publicacion_meli['codigo_retorno'] == 0)
								{
									...
									update_post_meta($id_post_publicacion, 'property_agent', $id_post_agencia_agente );
									...
									if (isset($vector_publicacion_meli->video_id))
									{
										if ($vector_publicacion_meli->video_id != '')
										{
											$vector_video = explode(";", $vector_publicacion_meli->video_id);
											if ($vector_video[1] == 'matterport')
											{
												update_post_meta($id_post_publicacion, 'embed_virtual_tour', $vector_video[0] );
											}
											else
											{
												update_post_meta($id_post_publicacion, 'embed_video_id', $vector_video[0] );
												update_post_meta($id_post_publicacion, 'embed_video_type', $vector_video[1] );
											}
										}
									}
									...
									foreach ($vector_publicacion_meli->attributes as $atributo)
									{
										foreach ($this->vectorCamposWpMeli as $wpMeli)
										{
											...
											{
												if ($atributo->id == 'TOTAL_AREA' || $atributo->id == 'COVERED_AREA')
												{
													$cantidad_area = explode(" ", $atributo->value_name);
													update_post_meta($id_post_publicacion, $wpMeli['wp'], $cantidad_area[0] );
												}
												else
												{
									...
									Si max_size contiene 800x600 se trata de fotos que fueron redimensionadas por Ofiliaria
									{
										"id": "933115-MLU89510186984_082025",
										"url": "http://http2.mlstatic.com/D_933115-MLU89510186984_082025-O.jpg",
										"secure_url": "https://http2.mlstatic.com/D_933115-MLU89510186984_082025-O.jpg",
										"size": "500x375",
										"max_size": "800x600",
										"quality": ""
									},
									*/
									foreach ($vector_publicacion_meli->pictures as $picture)
									{
										...
										$data['url_imagen'] = $picture->secure_url;
						...
						update_user_meta($id_autor, '_ofiliaria_publicaciones_importadas_meli', $vector_respuesta);
					...
					else
					{
						exit(json_encode(array(
							'codigo_retorno' => 4,
							'mensaje' => 'codigo_retorno '.$respuesta_buscar_publicaciones_meli['codigo_retorno'].' mensaje: '.$respuesta_buscar_publicaciones_meli['mensaje'],
							'publicaciones_sincronizadas' => $publicaciones_sincronizadas,
							'publicaciones_anteriores' => $publicaciones_anteriores
						), JSON_FORCE_OBJECT));	
					}
	...
	public function buscar_publicaciones_meli($id_usuario = null, $url = null)
	{
		$token_meli = '';
		$codigo_retorno = 0;
		...
		if ($codigo_retorno == 0)
		{
			...
			$respuesta = curl_exec($curl);
			curl_close($curl);
			update_user_meta($id_usuario, '_ofiliaria_respuesta_buscar_publicaciones_meli', $respuesta);
			$vector_publicaciones_meli = json_decode($respuesta);
			if (empty($vector_publicaciones_meli->results))
			{
				$codigo_retorno = 3;
				$mensaje = 'No se consiguieron publicaciones de Mercado Libre para el usuario';
			}
		}
		$respuesta = 
			[
	...
	public function obtener_publicacion_meli($token_meli = null, $codigo_publicacion = null)
	{
		...
		$respuesta = curl_exec($curl);
		curl_close($curl);
		update_user_meta($id_usuario, '_ofiliaria_respuesta_buscar_publicacion_meli', $respuesta);
	...
	{
		// Se comentó la función por seguridad si se ejecuta la acción por accidente, para importar los archivos descomentar la función
		/*
		global $wpdb;
		$nombre_tabla = $wpdb->prefix.'imagenes_publicaciones';
	...
	{
		// Se comentó la función por seguridad, si se ejecuta por accidente. Descomentar para que se ejecute
		/*
		global $wpdb;
		$nombre_tabla = $wpdb->prefix.'imagenes_publicaciones';		
	...
	public function verificar_usuario_conectado()
	{
		...
		if (isset($usuario_conectado->ID))
		{
			...
			if ( $registro_laravel == "")
			{		
				$this->crear_usuario_laravel($usuario_conectado->user_login, $usuario_conectado);
			}
	...
	{		
		...
		{
			$descripcion_iconv = iconv('UTF-8', 'ASCII//TRANSLIT', $_POST['wpestate_description'] );
			$descripcion_sin_interrogacion = str_replace('?', '', $descripcion_iconv);
			$patron = '/<[^>]*>/';
			$sustituto = '';
			$descripcion_texto_plano = preg_replace($patron, $sustituto, $descripcion_sin_interrogacion);
		}
		else
		{
			$codigo_retorno = 1;
		...
		if (!empty($_POST['ofiliaria_publicar_meli']))
        {
			...
			if ($resultados_paquetes_activos_meli['codigo_retorno'] > 0)
			{ 
				$codigo_retorno = 1;
				$errores[] = 'El usuario no tiene un paquete activo de publicaciones de Mercado Libre';
			}
			...
			foreach ($campos_generales as $indice => $campo_general)
			{
				...
				{
					update_user_meta($usuario->ID, '_ofiliaria_campo_general_requerido_'.$indice, $indice);
					if ($indice != 'stock')	
					{
						foreach ($this->vectorCamposWpMeli as $wpMeli)
						{
			...
			if (isset($_POST['property_area_submit']))
			{
				...
				elseif (isset($_POST['property_city_submit']))
				{
			...
			if ($nombre_barrio != '')
			{
				$termino_barrio = get_term_by('name', $nombre_barrio, 'property_area');
				$neighborhood = $termino_barrio->description;
			}
			elseif ($nombre_ciudad != '')
			{
				$termino_ciudad = get_term_by('name', $nombre_ciudad, 'property_city');
			...
			foreach ($campos_a_mostrar as $indice_campo_a_mostrar => $campo_a_mostrar)
			{
				if ($campo_a_mostrar['campo_requerido'] == 0)
				{
					...
					{
						if (!empty($_POST[$indice_campo_a_mostrar]) || $_POST[$indice_campo_a_mostrar] == $campo_a_mostrar['valor_minimo_permitido'])
						{
							if ($_POST[$indice_campo_a_mostrar] != 'No disponible')
							{
								if (isset($campo_a_mostrar['unidades_permitidas']))
								{
			...
			update_user_meta($usuario->ID, '_ofiliaria_vector_campos_especificos', $vector_campos_especificos);
		}
	 ...
	 */
	public function envio_publicacion_meli($usuario, $identificador, $id_post_publicacion, $id_publicacion_meli, $estatus_publicacion_meli)
	{
		...
		if (!empty($parametros))
		{
			...
			// Modificamos la condición para incluir la bandera de publicación cerrada
			if ($id_publicacion_meli == 0 || $indicador_editar_publicacion == 1 || $es_publicacion_cerrada)
			{
				...
				$vector_campos_meli['video_id'] = $embed_virtual_tour.';matterport';
				...
				update_user_meta($usuario->ID, '_ofiliaria_agregar_modificar_inmueble_meli_solicitud', $json_vector_campos_meli );
				...
				if ($respuesta_obtener_token_meli['codigo_retorno'] == 0)
				{
					...
					// --- LÓGICA DE ENDPOINT Y MÉTODO MODIFICADA ---
					$url = '';
					$metodo_http = '';
					...
					// El endpoint de republicación puede requerir un JSON con menos campos.
					// Sin embargo, enviar el JSON completo es seguro y funciona para esta lógica.
					curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($vector_campos_meli));
					
					$respuesta = curl_exec($curl);
					curl_close($curl);
					update_user_meta($usuario->ID, '_ofiliaria_agregar_modificar_inmueble_meli_respuesta', $respuesta );
					$respuesta_objeto = json_decode($respuesta);
					...
					if (isset($respuesta_objeto->id))
					{
						...
						if ($id_publicacion_meli != 0 && !$es_publicacion_cerrada)
						{
							$url_descripcion = 'https://api.mercadolibre.com/items/'.$id_publicacion_meli.'/description?api_version=2';
							$metodo_descripcion = "PUT";
						} else {
							// En nuevas publicaciones y republicaciones, usamos el nuevo ID.
							$url_descripcion = 'https://api.mercadolibre.com/items/'.$respuesta_objeto->id.'/description';
							$metodo_descripcion = "POST";
						}
	
						curl_setopt($curl, CURLOPT_URL, $url_descripcion);
						curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
			
						if ($metodo_descripcion == 'POST')
						{
							curl_setopt($curl, CURLOPT_POST, true);
						}
						else
						...
						curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($datos_descripcion));
						$respuesta = curl_exec($curl);
						curl_close($curl);
						update_user_meta( $usuario->ID, '_ofiliaria_agregar_modificar_descripcion_meli_respuesta', $respuesta );
						$respuesta_objeto = json_decode($respuesta);
					...
					else
					{
						...
						if (isset($respuesta_objeto->cause) && is_array($respuesta_objeto->cause) && !empty($respuesta_objeto->cause)) {
							$error_list_html .= '<ul>';
							foreach ($respuesta_objeto->cause as $error_cause) {
								if (isset($error_cause->message)) {
									$error_list_html .= '<li>' . esc_html($error_cause->message) . '</li>';
								}
							}
							$error_list_html .= '</ul>';
						}
						...
						$final_message = !empty($error_list_html) ?
							__('Mercado Libre no procesó la publicación por los siguientes motivos:', 'wpresidence') . $error_list_html :
							__('Error: Mercado Libre no procesó la publicación debido a un error en los datos enviados.', 'wpresidence');
	...
	public function eliminar_publicacion_meli($id_post_publicacion, $usuario)
	{
		...
		if ($respuesta_obtener_token_meli['codigo_retorno'] == 0)
		{
			...
			{
				update_user_meta($usuario->ID, '_ofiliaria_publicacion_no_tiene_id_meli_publicacion', $id_post_publicacion);
			}
		}
	}
	...
	public function crear_xml_infocasas()
	{
		...
		$area = '';
		$codigo_zona = 0;
		$latitud_publicacion = 0;
		$longitud_publicacion = 0;
			...
			{	
				...
				foreach ($publicaciones as $indice_publicacion => $publicacion)
				{
					...
					if ($indicador_publicar_infocasas === 'Sí') 
					{
						...
						$categoria_inmueble_infocasas = $categorias_meli_infocasas[$categoria_publicacion];

						$tipo_propiedad = $documento->createElement('tipoPropiedad');
						$tipo_propiedad = $propiedad->appendChild($tipo_propiedad);
						$texto_tipo_propiedad = $documento->createTextNode($infocasas_tipo_propiedad[$categoria_inmueble_infocasas]);
						...
						$tipo_operacion_infocasas = $operacion_meli_infocasas[$tipo_operacion_publicacion];

						$tipo_operacion = $documento->createElement('tipoOperacion');
						$tipo_operacion = $propiedad->appendChild($tipo_operacion);
						$texto_tipo_operacion = $documento->createTextNode($infocasas_tipo_operacion[$tipo_operacion_infocasas]);
						...
						foreach ($campos_a_procesar as $nombre_campo_xml => $valor_campo) 
						{
							...
							if ($meta_key == null)
							{
								if (isset($excepciones_datos_infocasas[$nombre_campo_xml]))
								{
									$meta_key = $excepciones_datos_infocasas[$nombre_campo_xml];
								}
							}
								...
								switch ($nombre_campo_xml) 
								{
									...
									case 'disposicion':
										...
										switch ($meta_value) {
											...
											case 'Interior': $valor_infocasas = 4; break;
										...
										case 'imagenes': 
											...
											if (!(empty($imagenes_enviar)))
											{
												foreach ($imagenes_enviar as $indice => $imagen)
												{
													...
													// $texto_url = $url->appendChild($texto_url);
				...
				$ruta_xml_usuario = $wp_filesystem->wp_content_dir().'uploads/infocasas/ofiliaria_'.$id_usuario_infocasas.'/ofiliaria_'.$_POST['id_usuario_infocasas'].'.xml';
		...
		[
			...
			'user_id' => intval($_POST['id_usuario_infocasas']),
			'directorio_post_xml' => $directorio_post_xml
		];
		exit(json_encode($resultado));	
	}
	...
	{
		// Inicio parámetros
		$id_usuario = null; // Se colocó null por seguridad si la acción se ejecuta por accidente
		$indicador_verificacion_proceso_descripcion_ejecutado = 'No';
	...
	{	
		...
		$codigo_retorno_meli = '';
		$mensaje_error_meli = '';
		$respuesta_objeto_meli = '';
		$respuesta_guardar_token = '';
		...
		if ($refresh_token != '')
		{
			...
			$respuesta_meli = curl_exec($curl);
			$codigo_retorno_meli = curl_getinfo($curl, CURLINFO_HTTP_CODE);
			if (curl_errno($curl)) 
			{
				$codigo_retorno = 2;
				$mensaje = 'Hubo un error en la respuesta de Mercado Libre';
				$mensaje_error_meli = curl_error($curl);
			} 
			else 
			{
				$respuesta_objeto_meli = json_decode($respuesta_meli);
				$token_meli = $respuesta_objeto_meli->access_token;
				$refresh_token = $respuesta_objeto_meli->refresh_token;
				$respuesta_guardar_token = $this->guardar_token_meli($token_meli, $refresh_token, $id_usuario);
				$codigo_retorno = 0;
		...
		$respuesta = 
			[
				'codigo_retorno' => $codigo_retorno,
				'mensaje' => $mensaje,
				'codigo_retorno_meli' => $codigo_retorno_meli,
				'mensaje_error_meli' => $mensaje_error_meli,
				'respuesta_objeto_meli' => $respuesta_objeto_meli 
			];
	...
	public function reasignacion_de_agente()
	{
		...
		// 2. Obtener los datos del agente para Mercado Libre
		...
		// 3. Obtener el ID de la publicación de Mercado Libre
		...
		// 4. Refrescar el token de Mercado Libre usando el ID del usuario actual
		...
		// 5. Construir el payload para la API de Mercado Libre
		$datos_put_meli = [
			'seller_contact' => [
		...
		// 6. Enviar la solicitud PUT a la API de Mercado Libre
		...
		if ($respuesta_meli_put['error'])
		{
			$codigo_retorno = 7; // Cambié el código de retorno para mantener la secuencia
			$mensaje = 'Error al actualizar datos en Mercado Libre: ' . ($respuesta_meli_put['mensaje'] ?? 'Error desconocido');
			if (isset($respuesta_meli_put['respuesta_api']['message']))
			{
				$mensaje .= ' - ' . $respuesta_meli_put['respuesta_api']['message'];
			}
			exit(json_encode(['codigo_retorno' => $codigo_retorno, 'mensaje' => $mensaje]));
		}
			...
			if ($categoria->nivel_2 == $categoria_inmueble)
			{
				if ($categoria->nivel_3 == $tipo_operacion)
				{
					...
					if ($indicador_categoria_encontrada == 1)
					{
						...
						foreach ($campos_especificos as $indice_campo_especifico => $campo_especifico)
						{
							$respuesta_campos_a_mostrar = $this->campos_a_mostrar($campo_especifico['id'], $campos_a_mostrar);
							$campos_a_mostrar = $respuesta_campos_a_mostrar;
						}
						foreach ($this->vector_campos_wp_meli_excepciones as $indice_campo_excepcion => $campo_excepcion)
						{
							if ($campo_excepcion['categoria_inmuebles'] == $categoria_inmueble && $campo_excepcion['tipo_operacion'] == $tipo_operacion)
							{
								$respuesta_campos_a_mostrar = $this->campos_a_mostrar($campo_excepcion['meli'], $campos_a_mostrar);
								$campos_a_mostrar = $respuesta_campos_a_mostrar;
							}
						}
						break;
	...
	public function campos_a_mostrar($id_campo_especifico = null, $campos_a_mostrar = null)
	{
		foreach ($this->vectorCamposWpMeli as $indiceWpMeli => $wpMeli)
		{
			$html_comparacion = '';
			$campo_requerido = 0;
			if ($id_campo_especifico == $wpMeli['meli'])
			{
				...
				$datos_campos_a_mostrar['meli'] = $id_campo_especifico;
				$datos_campos_a_mostrar['campo_requerido'] = $campo_requerido;
				$datos_campos_a_mostrar['valor_minimo_permitido'] = $wpMeli['valor_minimo_permitido'];

				if (isset($wpMeli['informacion_adicional']['opciones']))
				{
					$datos_campos_a_mostrar['opciones'] = $wpMeli['informacion_adicional']['opciones']; 
				}
				if (isset($wpMeli['informacion_adicional']['unidades_permitidas']))
				{
					$datos_campos_a_mostrar['unidades_permitidas'] = $wpMeli['informacion_adicional']['unidades_permitidas']; 
				}
	...
	private function enviar_solicitud_meli($punto_final, $metodo = 'GET', $datos = [], $requiere_autenticacion = false, $token_acceso = null)
	{
		...
		if ($error_curl)
		{
			return [
				'error' => true,
				'mensaje' => 'Error de cURL: ' . $error_curl,
				'codigo' => $codigo_http
			];
		}

		$datos_respuesta = json_decode($respuesta_curl, true);
		...
		else
		{
			return [
				'error' => true,
				'codigo' => $codigo_http,
				'mensaje' => 'Error de la API de Mercado Libre',
				'respuesta_api' => $datos_respuesta
			];
		}
	}
	...
	public function procesar_publicacion_agregar_whatsapp()
	{
		...
		if ($respuesta_meli['error'])
		{
			$codigo_retorno = 2; // Error tipo 2: Error al consultar ML
			$mensaje = 'Error al consultar la publicación en Mercado Libre: ' . $respuesta_meli['mensaje'];
			if (isset($respuesta_meli['respuesta_api']['message']))
			{
				$mensaje .= ' - Detalle: ' . $respuesta_meli['respuesta_api']['message'];
			}
		}
		else
		{
			$datos_publicacion_meli = $respuesta_meli['datos'];
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								if ($respuesta_put_meli['error'])
								{
									$codigo_retorno = 7; // Error tipo 7: Error al actualizar ML
									$mensaje = 'Error al actualizar la publicación en Mercado Libre: ' . $respuesta_put_meli['mensaje'];
									if (isset($respuesta_put_meli['respuesta_api']['message']))
									{
										$mensaje .= ' - Detalle: ' . $respuesta_put_meli['respuesta_api']['message'];
									}
								}
								else
								{
									$mensaje = 'Número de WhatsApp agregado/actualizado exitosamente en la publicación de Mercado Libre.';
								}
							}
						}
					}
	...
	public function pausar_activar_publicacion_meli($id_post_publicacion, $nuevo_estatus, $usuario)
	{
		...
		if ($respuesta_api_meli_obtener_estatus['error'])
		{
			$codigo_retorno = 4; // Error al consultar el estatus en Mercado Libre
			$mensaje = 'Error al consultar el estatus de la publicación en Mercado Libre: ' . 
						($respuesta_api_meli_obtener_estatus['mensaje'] ?? 'Error desconocido') .
						(isset($respuesta_api_meli_obtener_estatus['respuesta_api']['message']) ? ' - ' . $respuesta_api_meli_obtener_estatus['respuesta_api']['message'] : '');
			return ['codigo_retorno' => $codigo_retorno, 'mensaje' => $mensaje];
		}

		$estatus_actual_meli = $respuesta_api_meli_obtener_estatus['datos']['status'] ?? 'desconocido';
		...
		if ($nuevo_estatus === 'publish') // Intentar ACTIVAR la publicación
		{
			if ($estatus_actual_meli === 'paused')
			{
				...
				if ($respuesta_api_meli_actualizar['error'])
				{
					$codigo_retorno = 5; // Error al activar la publicación
					$mensaje = 'Error al activar la publicación en Mercado Libre: ' . 
								($respuesta_api_meli_actualizar['mensaje'] ?? 'Error desconocido') .
								(isset($respuesta_api_meli_actualizar['respuesta_api']['message']) ? ' - ' . $respuesta_api_meli_actualizar['respuesta_api']['message'] : '');
					return ['codigo_retorno' => $codigo_retorno, 'mensaje' => $mensaje];
				}
